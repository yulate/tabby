...
//                e.printStackTrace();
     ...
     */
    @Async("tabby-collector")
    public void collectSP(MethodReference methodRef, DataContainer dataContainer, TickTock tickTock) {
        String uuid = UUID.randomUUID().toString();
        try {
            if (methodRef.isIgnore() || methodRef.isInitialed() || methodRef.isBodyParseError()) {
                tickTock.countDown();
            ...
            JavaSootMethod method = methodRef.getSootMethodSP();
            if (method == null) {
                tickTock.countDown();
                return;
            }
            ...
            if (methodRef.isDao() || methodRef.isRpc() || methodRef.isMRpc()) {
                // TODO 对于上面3种类型，不进行分析，直接赋予action
                tickTock.countDown();
                return;
            }
            log.debug(method.getSignature().toString());
